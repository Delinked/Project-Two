...
Cat.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
meat.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

var wobbleSpeed = 3, wobbDist = 0.4;
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Player loses
  if (type == "lava" || type == "cat" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == 'meat') {
    ...
    // If there aren't any meats left, player wins
    if (!this.actors.some(function(actor) {
          return actor.type == "meat" ;
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
